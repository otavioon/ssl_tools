...
class CPC(pl.LightningModule, Configurable):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Just rename the batch to sample
        sample = batch
        ...
        sample = sample[
            :,
            max(0, (random_timestamp - 20 * self.window_size)) : \
            min(sample.shape[-1], random_timestamp + 20 * self.window_size)
        ]
        ...
        windowed_sample = np.split(
            ary=sample[:, : (sample_length // self.window_size) * self.window_size],
        ...
        density_ratios = torch.bmm(
            ...
            densities.expand_as(encodings)
            .unsqueeze(-1),
        )
        density_ratios = density_ratios.view(-1,)
